const data = [

  {
    photographer: "Fidan Nazim Qizi",
    src: {
      large:
        "https://images.pexels.com/photos/12155886/pexels-photo-12155886.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "An evocative photograph blending soft tones and intricate textures to tell a unique story of beauty and connection.",
  },
  {
    photographer: "ἐμμανυελ  ",
    src: {
      large:
        "https://images.pexels.com/photos/12182835/pexels-photo-12182835.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A mesmerizing nighttime shot filled with moody aesthetics, vibrant hues, and a deep connection to the urban nightscape.",
  },
  {
    photographer: "Eugene Golovesov",
    src: {
      large:
        "https://images.pexels.com/photos/12180544/pexels-photo-12180544.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "This artistic photograph portrays abstract yet captivating patterns with a focus on dynamic composition and surreal, dream-like color palettes.",
  },
  {
    photographer: "Malik Skydsgaard",
    src: {
      large:
        "https://images.pexels.com/photos/12132217/pexels-photo-12132217.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "Beautifully composed, this minimalist scene incorporates subtle details with harmonious lighting for a clean and balanced artistic expression.",
  },
  {
    photographer: "op23",
    src: {
      large:
        "https://images.pexels.com/photos/12166061/pexels-photo-12166061.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A stunning urban moment, frozen in time, blending sharp details and a contemplative aura. Perfectly crafted for cityscape lovers.",
  },
  {
    photographer: "Nina  Hill",
    src: {
      large:
        "https://images.pexels.com/photos/12168556/pexels-photo-12168556.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A gentle representation of simplicity, combining earthy tones and elegant details to deliver a uniquely charming visual narrative.",
  },
  {
    photographer: "Jonathan Borba",
    src: {
      large:
        "https://images.pexels.com/photos/12173281/pexels-photo-12173281.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "Energetic and vivid, this vibrant portrait captures a joyous atmosphere filled with life, warmth, and storytelling through dynamic framing.",
  },
  {
    photographer: "Enrique Hoyos",
    src: {
      large:
        "https://images.pexels.com/photos/12172737/pexels-photo-12172737.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A reflective masterpiece showcasing bold contrasts, sharp textures, and emotional undertones, creating a deep and impactful photographic experience.",
  },
  {
    photographer: "Lucía Montenegro",
    src: {
      large:
        "https://images.pexels.com/photos/12185664/pexels-photo-12185664.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A peaceful natural scene with soft lighting, showcasing tranquil colors and harmonious textures. A perfect representation of serenity.",
  },
  {
    photographer: "Veronika Bykovich",
    src: {
      large:
        "https://images.pexels.com/photos/12185506/pexels-photo-12185506.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "Captivating minimalist artwork with romantic vibes, highlighting a soft contrast between light and shadows in an intimate setting.",
  },

  {
    photographer: "Philip Warp",
    src: {
      large:
        "https://images.pexels.com/photos/12151766/pexels-photo-12151766.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A dreamy composition highlighting natural elements through a unique perspective. Soft light enhances the elegance of the photograph.",
  },
  {
    photographer: "Poyee Tsang",
    src: {
      large:
        "https://images.pexels.com/photos/12139617/pexels-photo-12139617.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A carefully framed scene showcasing warmth and character, featuring a blend of serene details and a sense of nostalgia.",
  },
  {
    photographer: "Poyee Tsang",
    src: {
      large:
        "https://images.pexels.com/photos/12139755/pexels-photo-12139755.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A vibrant urban moment with rich details, highlighting the essence of city life and the beauty within its chaos.",
  },
  {
    photographer: "Blue Arauz",
    src: {
      large:
        "https://images.pexels.com/photos/12151090/pexels-photo-12151090.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A mesmerizing natural landscape photograph, emphasizing lush greenery, tranquil waters, and soothing lighting for an immersive visual journey.",
  },
  {
    photographer: "Eugene Golovesov",
    src: {
      large:
        "https://images.pexels.com/photos/12142950/pexels-photo-12142950.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A creative exploration of shapes and reflections, balancing abstract elements with striking color contrasts and textured highlights.",
  },
  {
    photographer: "Martino Grua",
    src: {
      large:
        "https://images.pexels.com/photos/12142729/pexels-photo-12142729.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A rustic composition featuring earthy tones and architectural details, capturing the timeless charm of weathered elegance.",
  },
  {
    photographer: "Malik Skydsgaard",
    src: {
      large:
        "https://images.pexels.com/photos/12132204/pexels-photo-12132204.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A visually soothing composition combining clean lines, minimalist aesthetics, and a quiet harmony between light and shadow.",
  },
  {
    photographer: "Malik Skydsgaard",
    src: {
      large:
        "https://images.pexels.com/photos/12132208/pexels-photo-12132208.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "This artistic shot highlights texture and form through minimalism, creating a calming visual experience with elegant simplicity.",
  },
  {
    photographer: "Alyona Pastukhova",
    src: {
      large:
        "https://images.pexels.com/photos/12136490/pexels-photo-12136490.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A rich and vibrant photo showcasing natural beauty with a dramatic perspective, focusing on intricate details and lively colors.",
  },
  {
    photographer: "Rizky Sabriansyah",
    src: {
      large:
        "https://images.pexels.com/photos/12128696/pexels-photo-12128696.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
  },
  {
    photographer: "Kris Møklebust",
    src: {
      large:
        "https://images.pexels.com/photos/12143040/pexels-photo-12143040.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A powerful landscape with dramatic skies, blending natural beauty with intense colors to create a breathtaking scene.",
  },
  {
    photographer: "op23",
    src: {
      large:
        "https://images.pexels.com/photos/12125084/pexels-photo-12125084.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A dynamic cityscape, emphasizing the vibrancy of urban life with bold contrasts and intriguing compositions.",
  },
  {
    photographer: "7inchs",
    src: {
      large:
        "https://images.pexels.com/photos/12092806/pexels-photo-12092806.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "An abstract and colorful depiction of life’s subtle details, creatively framed to capture patterns and textures in nature.",
  },
  {
    photographer: "Taylor Marx",
    src: {
      large:
        "https://images.pexels.com/photos/12089012/pexels-photo-12089012.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A serene yet lively image portraying organic elements with a keen focus on vivid color contrasts and harmonious structure.",
  },
  {
    photographer: "Jair Hernandez",
    src: {
      large:
        "https://images.pexels.com/photos/12087895/pexels-photo-12087895.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A moment frozen in time, showcasing spontaneity and life’s beauty through carefully balanced natural lighting and subtle tones.",
  },
  {
    photographer: "Elena Rubtsova",
    src: {
      large:
        "https://images.pexels.com/photos/12082485/pexels-photo-12082485.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A soft, intimate portrayal of daily life, with delicate lighting and a focus on the beauty in ordinary details.",
  },
  {
    photographer: "Elena Rubtsova",
    src: {
      large:
        "https://images.pexels.com/photos/12082493/pexels-photo-12082493.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A visually appealing photograph showcasing pastel hues and intricate textures for a calming, meditative viewing experience.",
  },
  {
    photographer: "George Kascheev",
    src: {
      large:
        "https://images.pexels.com/photos/12052041/pexels-photo-12052041.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A vibrant blend of colors and movement, masterfully capturing the essence of a lively urban or natural moment.",
  },
  {
    photographer: "Trinity LaKose",
    src: {
      large:
        "https://images.pexels.com/photos/12050692/pexels-photo-12050692.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A dramatic photograph emphasizing light and shadow interplay to create a mysterious yet captivating narrative.",
  },
  {
    photographer: "Omar Zetina",
    src: {
      large:
        "https://images.pexels.com/photos/12046302/pexels-photo-12046302.jpeg?auto=compress&cs=tinysrgb&h=650&w=940",
    },
    details:
      "A breathtaking composition capturing raw emotion and storytelling through vibrant colors, dynamic framing, and timeless elegance.",
  },
];

export default data;
